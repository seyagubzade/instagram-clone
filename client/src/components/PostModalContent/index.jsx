...
import { addComment, likePost } from "../../store/posts/api_action";
...
const PostModalContent = ({
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const { text } = values;
      const { _id } = modalItem;
      dispatch(addComment({ text, username, id: _id }));
    ...
    <ModalContentContainer>
      <Row gutter={[16, 16]}>
        <Col xs={{ span: 24 }} md={{ span: 12 }}>
          <ModalImage src={modalItem.imageURL} alt={modalItem.caption} />
        </Col>
        <Col xs={{ span: 24 }} md={{ span: 12 }}>
          <ModalDetails>
            <div>
              <AuthorName>
                <p>
                  <span>{modalItem.author.name}</span> {modalItem.caption}
                </p>
              </AuthorName>{" "}
              <CreationTime>
                {new Date(modalItem.createdAt).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "short",
                  day: "2-digit",
                })}
              ...
              <LikesComments>
                ...
                >
                  <Icon name={"notification"} /> {modalItem.likes.length}
                </button>{" "}
                <button
                  style={{
                    background: "none",
                ...
                >
                  <Icon name={"comment"} />
                  {modalItem.comments.length}
                </button>
              </LikesComments>
              <CommentsList>
                {modalItem.comments.map((comment) => (
